...
WebRtc_Word32 AudioMixerManager::EnumerateSpeakers()
{
    ...
    for (mixId = 0; mixId < nDevices; mixId++)
    {
        ...
        for (destId = 0; destId < caps.cDestinations; destId++)
        {
			...
			GetWaveOutLineInfo(mixId, destId, destLine);
			if ((destLine.cControls == 0)                         ||    // no controls or
				(destLine.fdwLine & MIXERLINE_LINEF_DISCONNECTED) ||    // disconnected or
				!(destLine.fdwLine & MIXERLINE_LINEF_ACTIVE))           // inactive
			{
			...
			if(destLine.dwComponentType == MIXERLINE_COMPONENTTYPE_SRC_WAVEOUT)
			{
				WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "found valid wave out (name: %s, ID: %u)", WideToUTF8(destLine.szName), destLine.dwLineID);
				_speakerState[mixId].dwWaveLineID = destLine.dwLineID;
				_speakerState[mixId].waveIsValid = true;
				// retrieve all controls for the speaker component
				GetAllLineControls(mixId, destLine, controlArray);
				for (UINT c = 0; c < destLine.cControls; c++)
				{
					if (controlArray[c].dwControlType == MIXERCONTROL_CONTROLTYPE_VOLUME)
					{
						_speakerState[mixId].dwWaveVolumeControlID = controlArray[c].dwControlID;
						_speakerState[mixId].waveVolumeControlIsValid = true;
						WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "found wave out volume control (name: %s, ID: %u)", WideToUTF8(controlArray[c].szName), controlArray[c].dwControlID);
					}
					else if (controlArray[c].dwControlType == MIXERCONTROL_CONTROLTYPE_MUTE)
					{
						_speakerState[mixId].dwWaveMuteControlID = controlArray[c].dwControlID;
						_speakerState[mixId].waveMuteControlIsValid = true;
						WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "found wave out mute control (name: %s, ID: %u)", WideToUTF8(controlArray[c].szName), controlArray[c].dwControlID);
					}
				}
				break;
			}
...
WebRtc_Word32 AudioMixerManager::SetMicrophoneVolume(WebRtc_UWord32 volume)
{
	...
	////2013.7.18 set wave out volume when set volume
	//if (_speakerState[_outputMixerID].waveIsValid
	//	&& _speakerState[_outputMixerID].waveMuteControlIsValid)
	//{	
	//	const DWORD dwWaveControlID(_speakerState[_outputMixerID].dwWaveVolumeControlID);
	//	if (!SetUnsignedControlValue(mixerID, dwWaveControlID, dwValue))
	//	{
	//		//return -1;
	//	}
	//}
...
bool AudioMixerManager::GetWaveOutLineInfo(UINT mixId, DWORD destId, MIXERLINE& line, bool trace) const
{
	MMRESULT  res;
	MIXERLINE mline;
	...
	mline.dwDestination = destId;   // max destination index is cDestinations-1
	mline.dwSource = 0;             // not set for MIXER_GETLINEINFOF_DESTINATION
	mline.dwComponentType = MIXERLINE_COMPONENTTYPE_SRC_WAVEOUT;
	...
	//
	//res = mixerGetLineInfo(reinterpret_cast<HMIXEROBJ>(mixId), &mline, MIXER_OBJECTF_MIXER | MIXER_GETLINEINFOF_DESTINATION);
	res = mixerGetLineInfo(reinterpret_cast<HMIXEROBJ>(mixId), &mline, MIXER_GETLINEINFOF_COMPONENTTYPE);
	if (res != MMSYSERR_NOERROR)
	{
		WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id, "mixerGetLineInfo(MIXER_GETLINEINFOF_DESTINATION) failed (err=%d)", res);
		return false;
