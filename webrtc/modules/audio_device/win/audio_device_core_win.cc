...
bool AudioDeviceWindowsCore::CoreAudioIsSupported()
{
    ...
    if (!comInit.succeeded()) {
      ...
      // want to ensure that MTA is used and therefore return false here.
		//2013.6.18 by huajian.yin.解决com库已经被初始化，导致此处返回失败的问题。
		//return false;
    }
...
{
	bool defaultConsoleDevice(false);
	...
	{
		defaultConsoleDevice = true;
		index = 0;
		WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Default Console endpoint device will be used");
	}
    ...
    char guid[kAdmMaxGuidSize])
{
	...
	{
		ret = _GetDefaultDeviceName(eCapture, eConsole, szDeviceName, bufferLen);
	}	
    else
    {
	...
	{
		ret = _GetDefaultDeviceID(eCapture, eConsole, szDeviceName, bufferLen);
	}
    else
    {
