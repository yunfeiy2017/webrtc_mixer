    ...
    const WebRtc_UWord32 bufferSize)
{
 ...
 bufSize= %ld)",
    ...
    WebRtc_UWord32 bytesRequested = (codec_info_.channels == 2) ?
        totalBytesNeeded >> 1 : totalBytesNeeded;
	
	if(_bytesPerSample == 1)//output 16bit sample
	{
		bytesRequested<<=1;
	}
    ...
    WebRtc_Word32 bytesRead = ReadWavData(
        wav,
        _tempData,//(codec_info_.channels == 2) ? _tempData : (WebRtc_UWord8*)outData,
        totalBytesNeeded);
    if(bytesRead == 0)
    {
        return 0;
	...
	{  				
		...
		WebRtc_Word16* sampleData = (WebRtc_Word16*) outData;
		if(_bytesPerSample == 1)	
		{
			for (WebRtc_UWord32 i = 0; i < bytesRequested / 2; i++)
			{
				sampleData[i] = ((((_tempData[2 * i] + _tempData[(2 * i) + 1] + 1) >> 1) )-128)<<8;
			}
		}
        else
		{
			WebRtc_Word16* inData = (WebRtc_Word16*) _tempData;
			for (WebRtc_UWord32 i = 0; i < bytesRequested / 2; i++)
			{
				// Sample value is the average of left and right buffer rounded to
				// closest integer value. Note samples can be either 1 or 2 byte.
				sampleData[i] = ((inData[2 * i] + inData[(2 * i) + 1] +
								  1) >> 1);
			}
        }
    }
	else
	{
		if(_bytesPerSample == 1)
		{
			...
			for (WebRtc_UWord32 i = 0; i < bytesRequested / 2; i++)
			{
				sampleData[i] = (_tempData[i]-128)<<8;
			}
		}
		else
		{
