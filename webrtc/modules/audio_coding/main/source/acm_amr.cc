...
WebRtc_Word16 ACMAMR::DecodeSafe(WebRtc_UWord8* bitstream,
  ...
  *audio_samples = WebRtcAmr_Decode(decoder_inst_ptr_, (int16_t*)bitstream, bitstream_len_byte, audio);
  return *audio_samples*2;
}
...
WebRtc_Word16 ACMAMR::InternalInitEncoder(WebRtcACMCodecParams* codec_params) {
  ...
  status += (WebRtcAmr_EncoderInit(encoder_inst_ptr_, 0) < 0) ? -1 : 0;
