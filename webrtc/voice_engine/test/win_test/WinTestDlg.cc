...
BEGIN_MESSAGE_MAP(CWinTestDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_DEFAULTIN, &CWinTestDlg::OnBnClickedBtnDefaultin)
	ON_BN_CLICKED(IDC_BTN_DEFAULTOUT, &CWinTestDlg::OnBnClickedBtnDefaultout)
END_MESSAGE_MAP()
...
{
    std::string micFile = "./rec_mic_mono_16kHz.pcm";
...
void CWinTestDlg::OnBnClickedCheckAgc()
{
    ...
    const bool enable = (check == BST_CHECKED);
	//AgcConfig config;
	//_veApmPtr->SetAgcStatus(true, kAgcDefault);
	//if (enable)
	//{
	//	_veApmPtr->GetAgcConfig(config);
	//	config.digitalCompressionGaindB = 80;
	//	_veApmPtr->SetAgcConfig(config);
	//}
	//else
	//{
	//	_veApmPtr->GetAgcConfig(config);
...
void CWinTestDlg::OnBnClickedBtnDefaultin()
{
	_veHardwarePtr->SetRecordingDevice(-1);
}

void CWinTestDlg::OnBnClickedBtnDefaultout()
{
	_veHardwarePtr->SetPlayoutDevice(-1);
}
