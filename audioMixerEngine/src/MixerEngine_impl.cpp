...
#define PrepareForRecvFrom(_pIo) \
	...
	int MixerEngineImpl::newPacket(const void* audio_data,
		...
		const uint32_t sample_rate)
	{
		...
		if (pcmFile)
		{
			fwrite(audio_data, bytes_per_sample, number_of_frames*number_of_channels, pcmFile);
			fflush(pcmFile);
		}
		if (callback_)
			callback_->OnMixSamples(audio_data, number_of_frames, bytes_per_sample, number_of_channels, sample_rate);
